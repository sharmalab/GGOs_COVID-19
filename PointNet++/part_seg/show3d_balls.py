...
def onmouse(*args):
    ...
    changed=True
cv2.namedWindow('3DLungPoints')
cv2.moveWindow('3DLungPoints',0,0)
cv2.setMouseCallback('3DLungPoints',onmouse)
...
def showpoints(xyz,c_gt=None, c_pred = None ,waittime=0,showrot=False,magnifyBlue=0,freezerot=False,background=(0,0,0),normalizecolor=True,ballradius=10):
    ...
    def render():
        ...
        if showrot:
            ...
            cv2.putText(show,'zoom %d%%'%(int(zoom*100)),(30,showsz-70),0,0.5,(0,0,255))
    changed=True
    while True:
        if changed:
            render()
            changed=False
        cv2.imshow('3DLungPoints',show)
        if waittime==0:
        ...
        if cmd==ord('n'):
            ...
            cv2.imwrite('3DLungPoints.png',show)
        if waittime!=0:
            break
    return cmd
