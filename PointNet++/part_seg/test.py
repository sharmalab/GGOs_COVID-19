...
parser.add_argument('--category', default='Lung', help='Which single class to train on [default: Lung]')
parser.add_argument('--model', default='pointnet2_part_seg', help='Model name [default: pointnet2_part_seg]')
parser.add_argument('--model_path', default='log_epoch200/model.ckpt', help='model checkpoint file path [default: log/model.ckpt]')
...
DATA_PATH = os.path.join(ROOT_DIR, 'data', 'shapenetcore_partanno_v0')
...
def inference(sess, ops, pc, batch_size):
    ...
    num_batches = int(pc.shape[0]/batch_size/2)
